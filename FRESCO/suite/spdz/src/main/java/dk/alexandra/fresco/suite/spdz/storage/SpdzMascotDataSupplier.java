...
public class SpdzMascotDataSupplier implements SpdzDataSupplier {
  ...
  public SpdzTriple getNextTriple() {
    ensureInitialized();
    if (triples.isEmpty()) {
      logger.debug("Getting another triple batch");
      triples.addAll(mascot.getTriples(batchSize));
      logger.debug("Got another triple batch");
    }
    MultiplicationTriple triple = triples.pop();
    return MascotFormatConverter.toSpdzTriple(triple);
  }

  public void produceTriples(int size) {
    ensureInitialized();
    triples.addAll(mascot.getTriples(size));
  }
  ...
  public void produceRandomFieldElements(int size) {
    ensureInitialized();
    randomElements.addAll(mascot.getRandomElements(size));
  }

  public SpdzSInt shareKnownElement(FieldElement value) {
    ensureInitialized();
    return MascotFormatConverter.toSpdzSInt(mascot.shareKnownElement(value));
  }
  ...
  public SpdzSInt getNextBit() {
    ...
    if (randomBits.isEmpty()) {
      logger.debug("Getting another bit batch");
      randomBits.addAll(mascot.getRandomBits(batchSize));
      logger.debug("Got another bit batch");
    }
