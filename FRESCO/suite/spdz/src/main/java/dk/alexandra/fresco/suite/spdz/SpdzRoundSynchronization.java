...
import dk.alexandra.fresco.suite.spdz.ECCExtension.SpdzECCMacCheckProtocol;
...
public class SpdzRoundSynchronization implements RoundSynchronization<SpdzResourcePool> {
  ...
  protected void doMacCheck(SpdzResourcePool resourcePool, Network network) {
    ...
    OpenedValueStore<SpdzECPoint, ECPoint> ECCstore = ((SpdzResourcePoolImpl)resourcePool).getOpenedECCValueStore();
    // switched to SpdzECCMacCheck
    SpdzECCMacCheckProtocol macCheck = new SpdzECCMacCheckProtocol(
        store.popValues(),
        ECCstore.popValues(),
  ...
  public void finishedBatch(int gatesEvaluated, SpdzResourcePool resourcePool, Network network) {
    ...
    } else if (store.exceedsThreshold(openValueThreshold) || ECCstore.exceedsThreshold(openValueThreshold)) {
      doMacCheck(resourcePool, network);
      isCheckRequired = false;
    }
  }
  ...
  public void finishedEval(SpdzResourcePool resourcePool, Network network) {
    OpenedValueStore<SpdzSInt, FieldElement> store = resourcePool.getOpenedValueStore();
    OpenedValueStore<SpdzECPoint, ECPoint> ECCstore = ((SpdzResourcePoolImpl)resourcePool).getOpenedECCValueStore();
    if (store.hasPendingValues() || ECCstore.hasPendingValues()) {
      doMacCheck(resourcePool, network);
    }
  }
  ...
  public void beforeBatch(
    ...
    OpenedValueStore<SpdzSInt, FieldElement> store = resourcePool.getOpenedValueStore();
    OpenedValueStore<SpdzECPoint, ECPoint> ECCstore = ((SpdzResourcePoolImpl)resourcePool).getOpenedECCValueStore();
    if ((store.hasPendingValues() || ECCstore.hasPendingValues()) && isCheckRequired) {
      doMacCheck(resourcePool, network);
    }
  }
